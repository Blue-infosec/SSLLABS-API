...
class Manage_Elasticsearch():
    ...
    def create_index(self, index_name, mapping_schema=None):
        ...
        try:
           ...
           logger.info("Creating index %s in elasticsearch database" % self.el_index)
    ...
    def update_list(self, data, additional_info = None):
    ...
    def update_dict(self, data, additional_info = None):
        try:
                ip_data = dict()
                ip_data = {
                             'ts':datetime.today().utcnow().isoformat()
                          }
                ...
                # update with ssllabs-api results
                ip_data.update(data)
