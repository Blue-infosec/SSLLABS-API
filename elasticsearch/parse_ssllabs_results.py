...
from manage_elasticsearch import Manage_Elasticsearch
...
logger = logging.getLogger('ssllabs-api')
...
data = parse_results(json_ssllabs)
...
if not es_health:
    logger.info("""Failed to connect to Elasticsearch server. Kindly re-check Elasticsearch settings
                   and then try again""")
    sys.exit(1) 
...
logger.info("SSL Labs API results written to elasticsearch successfully.")
